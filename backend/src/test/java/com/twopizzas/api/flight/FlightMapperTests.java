...
public class FlightMapperTests {
    ...
    void test() {
        ...
        // WHEN
        FlightDto target = mapper.map(source);
        ...
        Optional<FlightDto.Seat> targetSeat = target.getSeats().stream().filter(s -> seat.getName().equals(s.getName())).findFirst();
    ...
    @Test
    @DisplayName("GIVEN valid flightSeatAllocation WHEN mapped THEN return valid PassengerDto")
    void test2() {
        // GIVEN
        Passenger passenger = Mockito.spy(new Passenger("givenName", "surname", LocalDate.now(), "passport", "nationality"));
        EntityId id = EntityId.nextId();
        Booking booking = Mockito.mock(Booking.class);
        Mockito.when(booking.getId()).thenReturn(id);
        Mockito.when(passenger.getBooking()).thenReturn(booking);
        FlightSeat seat = new FlightSeat("name", SeatClass.FIRST, Mockito.mock(Flight.class));
        FlightSeatAllocation source = new FlightSeatAllocation(seat, passenger);

        // WHEN
        FlightPassengerDto target = mapper.map(source);

        // THEN
        Assertions.assertEquals(source.getPassenger().getGivenName(), target.getGivenName());
        Assertions.assertEquals(source.getSeat().getName(), target.getSeatName());
        Assertions.assertEquals(source.getPassenger().getBooking().getId().getValue().toString(), target.getBooking());
        Assertions.assertEquals(source.getPassenger().getSurname(), target.getSurname());
        Assertions.assertEquals(source.getPassenger().getNationality(), target.getNationality());
        Assertions.assertEquals(source.getPassenger().getDateOfBirth(), target.getDateOfBirth());
        Assertions.assertEquals(source.getPassenger().getPassportNumber(), target.getPassportNumber());
    }
}
